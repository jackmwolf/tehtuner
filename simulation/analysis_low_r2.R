# ANALYZE SIMULATIONS WITH LOWER R2
...
# Only use jobs Rxx2 and Rxx4 for only randomforest and superlearner
cnums <- expand.grid("R", 1:2, 0:2, c(2,4)) %>% 
...
compact_results <- function(cnum, work_dir = file.path(), verbose = TRUE) {
  ...
  re_out$cnum <- str_extract(dir, "R\\d\\d\\d$")
...
re.df %>% 
  ...
  summarise_vt %>% 
  saveRDS(file.path(table_dir, paste0("vt_results_r2_", Sys.Date(), ".Rds")))